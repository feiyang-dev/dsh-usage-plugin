...
- **适配 DeepSeek V4.1 Flash 新模型与新价格**（`lib/index.js`）：官方新增模型名 `deepseek-flash`（DeepSeek-V4.1-Flash），并同步下调了 Flash 档位价格；本版按官方公布的价格表完成适配。

### 变更 / Changed

- **模型档位按官方新命名归并**：计费档位由 `deepseek-v4-flash` / `deepseek-v4-flash-vision-exp` / `deepseek-v4-pro` 三项合并为 **`deepseek-flash`** 与 **`deepseek-v4-pro`** 两项。旧模型名 `deepseek-v4-flash`、`deepseek-v4-flash-vision-exp` 仍可调用（官方已将其路由到 DeepSeek-V4.1-Flash 并按 Flash 单价计费），插件会把它们归入 `deepseek-flash` 档位一并计费，历史记录与新增记录口径一致。
- **价格表按生效时间分三段**（新增 `peakValleyV41` 档）：
  - `base` —— 旧基础价（2026-08-17 00:00 之前）；
  - `peakValley` —— 第一版峰谷价（2026-08-17 00:00 ~ 2026-09-10 12:00）；
  - `peakValleyV41` —— **V4.1 Flash 调价后的峰谷价（2026-09-10 12:00 北京时间起）**：
    - `deepseek-flash`：空闲 缓存命中 **0.02** / 输入 **1.0** / 输出 **4.0** 元，高峰为其两倍（0.04 / 2.0 / 8.0）；
    - `deepseek-v4-pro`：价格不变（空闲 0.15 / 4.5 / 13.5，高峰 0.3 / 9.0 / 27.0）。
- **计费按记录发生时刻取对应版本**：`auto` 档按调用时间选档；记录一旦产生，其费用不会随后续调价而改变（历史记录在旧档、新记录在新档），导出 CSV / 概览 / 用量日历 / 缓存命中列表口径一致。
- `list` API 新增 `effectiveV41At` 字段（V4.1 调价生效时间戳），`effectiveAt` 语义不变（第一版峰谷价生效时间），向后兼容。

### 界面 / UI

- **价格表页适配分段峰谷价**（`lib/client.js`）：「峰谷价」档展示**当前生效**的一版峰谷价表；新增两段说明——峰谷价表分段规则与生效时刻、旧模型名与 `deepseek-flash` 合并计费的口径。
- 生效状态提示升级：在「新价格表（峰谷价）已生效（自 2026-08-17 起）」之后追加 **DeepSeek V4.1 Flash 调价**的生效时间与价格变化说明（调价前提示「将于 … 生效」）。
- 价格表行标签更新为 `deepseek-flash` / `deepseek-v4-pro`，与官方模型名、DSH 模型选择器一致。
- 新增文案已同步补充英文词典（`__T_EN`），中英双语即时可用。

### 测试 / Tests

- 新增 `test/pricing.test.js`（8 条）：校验生效时刻常量（2026-08-17 00:00 / 2026-09-10 12:00 北京时间）、V4.1 Flash 新价与「空闲价 = 高峰价一半」规则、V4 Pro 价格不变、第一版峰谷价与旧基础价保留、`peakValleyTableOf` 切换边界（精确到生效前一毫秒）、新旧模型名归并、价格表完整性（每个档位 × 每个模型字段齐全）、`defaultPricing()` 每次返回独立副本。
- 为便于回归测试，`lib/index.js` 新增导出：`PEAK_VALLEY_AT`、`V41_EFFECTIVE_AT`、`PRICE_REGIMES`、`PRICE_MODELS`、`defaultPricing()`、`modelKeyOf()`、`peakValleyTableOf()`；插件运行行为不变（价格表仍是每个 `apply()` 实例私有副本）。
- `test/client-i18n.test.js` 新增用例：V4.1 调价相关的新文案在中英两种语言下都能正确解析（英文界面不残留中文）。
- 全部 **39/39** 通过。

### 说明 / Notes

- 依据官方价格页：旧模型名 `deepseek-v4-flash`、`deepseek-v4-flash-vision-exp` 已下线但暂时路由到 V4.1 Flash；`deepseek-v4-pro` 官方已明确在 2026-09-14 之后继续提供且计费方式不变，故本版不对 Pro 档做时间路由，价格维持不变。
- 未引入新 npm 依赖。
