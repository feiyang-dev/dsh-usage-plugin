  ...
  factory: (require) => {
    ...
    function BalancePanel() {
      ...
      var providers = [
        ...
        { id: "qwen-token-plan", name: "百炼 Token Plan", hint: "百炼控制台 OAuth 登录（~/.bailian/config.json 的 access_token，用 bl auth login --console 生成）", actionUrl: "https://bailian.console.aliyun.com/cli", actionLabel: "打开百炼 CLI 登录说明" }
      ];
      ...
      return el("div", { style: st.root },
        el("div", { style: st.head },
          el("div", null,
            ...
            el("div", { style: st.sub }, "查询 DeepSeek、SiliconFlow、DigitalOcean 与百炼 Token Plan")
          ),
          el("div", { style: st.actions },
        ...
        d && d.ok
          ? el("div", { style: st.hero },
              ...
              el("div", { style: st.heroValue }, isQwen ? d.totalBalance : symbol + " " + fmtBalance(d.totalBalance)),
        ...
        d && d.ok
          ? el("div", { style: st.cards },
              (d.details || []).map(function (item, idx) {
                return el(Card, { key: String(idx), label: item.label, value: isQwen ? item.value : symbol + " " + fmtBalance(item.value), hint: item.hint });
              }),
        ...
        provider === "qwen-token-plan"
          ? el("div", { style: Object.assign({}, st.infobox, { marginTop: 14 }) },
              el("div", { style: st.infoboxTitle }, "百炼 Token Plan 查询说明"),
              el("div", null, "本查询复用百炼 CLI（bl）的“控制台登录”OAuth token，不需要阿里云 AccessKey，权限面很小。"),
              el("div", { style: st.note, marginTop: 6 }, "如尚未登录，请先在终端执行：bl auth login --console，然后回到本页点“查询余额”。"),
              el("a", { style: st.actionLink, href: selected.actionUrl, target: "_blank", rel: "noreferrer" }, selected.actionLabel || "打开百炼 CLI 登录说明")
            )
          : null,
