window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var switchLang = function () { __LANG = __LANG === "en" ? "zh" : "en"; try { window.localStorage.setItem("dsh-usage-lang", __LANG); } catch (e) {} return __LANG; };
    ...
    var modelLabel = function (mk) { if (mk === "deepseek-v4-flash") return "deepseek-v4-flash"; if (mk === "deepseek-v4-pro") return "deepseek-v4-pro"; return t("未知模型"); };
    ...
    var dayLabel = function (key) { var p = String(key).split("-"); return p[0] + t("年") + (+p[1]) + t("月") + (+p[2]) + t("日"); };
    ...
    function OverviewView(props) {
      ...
      return el("div", null,
        ...
        el("div", { style: st.cards },
          ...
          el(Card, { label: t("高峰消耗"), value: fmtMoney(peakTotal), hint: t("高峰时段 9:00–12:00、14:00–18:00") }),
    ...
    function CalendarView(props) {
      ...
      var weeks = [t("一"), t("二"), t("三"), t("四"), t("五"), t("六"), t("日")];
      var cells = [];
      ...
      return el("div", null,
        el("div", { style: st.calBar },
          ...
          el("span", { style: { fontSize: fs(14), fontWeight: 600, minWidth: 120, textAlign: "center" } }, y + t("年") + (m + 1) + t("月")),
        ...
        el("div", { style: st.cards, marginTop: 12 },
          el(Card, { label: t("本月调用"), value: fmtInt(monthCalls), hint: t("高峰 ") + fmtInt(monthPeakCalls) + t(" · 空闲 ") + fmtInt(monthOffCalls) + " · " + y + t("年") + (m + 1) + t("月") }),
          ...
          el(Card, { label: t("高峰消耗"), value: fmtMoney(monthPeak), hint: t("高峰时段 9:00–12:00、14:00–18:00") }),
        ...
        el("div", { style: st.sec }, t("每日统计（") + y + t("年") + (m + 1) + t("月）")),
        monthRows.length === 0
    ...
    function PriceView(props) {
      ...
      return el("div", null,
        ...
        regime === "auto"
          ? el("div", { style: st.note, marginTop: 6 },
              t("自动模式：按调用时间自动选择计费档位——新价格表生效前的调用按基础价（旧价格表），生效后的调用按峰谷价（高峰时段 9:00–12:00、14:00–18:00 用高峰价，其余空闲时段用空闲价）。当前显示："),
        ...
        el("div", { style: st.note, marginTop: 8 },
          t("单位：元 / 百万 tokens。高峰时段（北京时间 9:00–12:00、14:00–18:00）用高峰价，其余空闲时段用空闲价（空闲价 = 高峰价的一半）。概览、用量日历、缓存命中列表中的消耗均已按高峰 / 空闲分列统计。")
        ),
    ...
    function UsagePanel(props) {
      ...
      function showExport(res) {
        if (res && res.ok) {
          setExportMsg(t("已导出：") + res.path);
          if (res.dir) api({ action: "reveal", dir: res.dir }).catch(function () {});
        } else {
          setExportMsg(t("导出失败：") + ((res && res.error) || t("未知错误")));
        }
      }

      function doExport(kind) {
        setExportMsg(t("导出中…"));
        api({ action: "export", kind: kind, dir: destDir || undefined }).then(showExport).catch(function (e) { setExportMsg(t("导出失败：") + String((e && e.message) || e)); });
      }

      function doExportImage() {
        setExportMsg(t("生成图片中…"));
        if (!canvasNode) { setExportMsg(t("画布不可用")); return; }
        try {
          drawReport(canvasNode, (data.records || []).slice());
          var dataUrl = canvasNode.toDataURL("image/png");
          api({ action: "exportPng", dataUrl: dataUrl, dir: destDir || undefined }).then(showExport).catch(function (e) { setExportMsg(t("导出失败：") + String((e && e.message) || e)); });
        } catch (e) {
          setExportMsg(t("生成图片失败：") + String((e && e.message) || e));
        }
      }

      function pickDestDir() {
        setExportMsg(t("打开目录选择…"));
        api({ action: "pickDir" }).then(function (res) {
          if (res && res.ok) { setDestDir(res.path); setExportMsg(t("导出目标：") + res.path); }
          else if (res && res.cancelled) { setExportMsg(""); }
          else { setExportMsg(t("选择目录失败：") + ((res && res.error) || "")); }
        }).catch(function (e) { setExportMsg(t("选择目录失败：") + String((e && e.message) || e)); });
      }

      function doReveal() {
        api({ action: "reveal", dir: destDir || "data" }).then(function (res) {
          if (!res || !res.ok) setExportMsg(t("打开文件夹失败：") + ((res && res.error) || ""));
        }).catch(function (e) { setExportMsg(t("打开文件夹失败：") + String((e && e.message) || e)); });
      }
      ...
      function onFileChange(e) {
        ...
        readPromise.then(function (content) {
          ...
          api({ action: "import", content: String(content), filename: f.name }).then(function (res) {
            if (res && res.ok) {
              setImportMsg(t("导入成功：新增 ") + res.imported + t(" 条，跳过重复 ") + res.skipped + t(" 条，忽略无效 ") + res.invalid + t(" 条，现有共 ") + res.total + t(" 条"));
              refresh();
            } else {
              setImportMsg(t("导入失败：") + ((res && res.error) || t("未知错误")));
            }
          }).catch(function (err) { setImportMsg(t("导入失败：") + String((err && err.message) || err)); });
        }).catch(function () { setImportMsg(t("读取文件失败")); });
      }
      ...
      return el("div", { style: st.root },
        ...
        el("div", { style: st.head },
          el("div", { style: st.headleft },
            ...
            el("span", { style: nowPeak ? st.badgePeak : st.badgeValley }, nowPeak ? t("当前 · 高峰时段") : t("当前 · 空闲时段")),
          ...
          el("div", { style: st.actions },
            ...
            el("button", { style: st.btn, title: "Language / 语言", onClick: function () { switchLang(); refresh(); } }, __LANG === "en" ? "中文" : "EN"),
        ...
        el("div", { style: st.actions },
          ...
          destDir ? el("button", { style: st.btn, onClick: function () { setDestDir(""); setExportMsg(t("已恢复默认数据目录")); } }, t("重置")) : null
        ),
        el("div", { style: st.actions },
    ...
    function BalancePanel() {
      ...
      function saveDigitalOceanCredential() {
        ...
        if (!value) {
          setSaving({ status: "error", error: t("请输入以 dop_v1_ 开头的 DigitalOcean 账户 PAT。") });
          return;
        }
      ...
      var heroLabel = d && d.balanceLabel ? d.balanceLabel : ((d && d.providerName) || selected.name) + t(" · 账户余额");
      ...
      return el("div", { style: st.root },
        el("div", { style: st.head },
          el("div", null,
            ...
            el("div", { style: st.sub }, t("查询 DeepSeek、SiliconFlow 与 DigitalOcean 账户余额"))
          ),
          el("div", { style: st.actions },
        ...
        s.status === "error"
          ...
          : null,
        s.status === "loading" && !d ? el("div", { style: st.empty }, t("正在查询余额…")) : null,
        d && d.ok
