...
function parseQwenTokenPlan(provider, data) {
  // 网关返回：{ per5HourPercentage, per5HourResetTime, per1WeekPercentage, per1WeekResetTime }
  const has5h = data && (data.per5HourPercentage != null || data.per5HourResetTime != null)
  const has1w = data && (data.per1WeekPercentage != null || data.per1WeekResetTime != null)
  if (!has5h && !has1w) return fail('百炼 Token Plan 响应中没有可识别的配额字段')
  const details = []
  if (has5h) {
    const p = fmtPct(data.per5HourPercentage)
    details.push({
      label: '5 小时配额',
      value: p == null ? '暂无信息' : '已用 ' + p,
      hint: p == null ? '可能是无限额，请到百炼控制台确认' : ('重置 ' + fmtResetEpoch(data.per5HourResetTime) || '未知')
    })
  } else {
    details.push({ label: '5 小时配额', value: '暂无信息', hint: '可能是无限额，请到百炼控制台确认' })
  }
  if (has1w) {
    const p = fmtPct(data.per1WeekPercentage)
    details.push({
      label: '1 周配额',
      value: p == null ? '暂无信息' : '已用 ' + p,
      hint: p == null ? '暂无信息' : ('重置 ' + fmtResetEpoch(data.per1WeekResetTime))
    })
  } else {
    details.push({ label: '1 周配额', value: '暂无信息', hint: '可能是无限额，请到百炼控制台确认' })
  }
  const primaryPct = details[0].value || details[1].value || '暂无'
  return result(provider, {
    currency: '',
    isAvailable: null,
    totalBalance: primaryPct,
    balanceLabel: '百炼 Token Plan 配额用量',
    details,
    sourceNote: '数据来自阿里云百炼控制台内部门户网关（zeldaHttp.apikeyMgr./tokenplan/personal/api/v2/usage），展示的是配额已用百分比与重置时间，不是剩余 token 数。'
  })
}
