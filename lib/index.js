  ...
  apply(ctx) {
    ...
    try {
      ...
      function costFor(rec, regime) {
        ...
        if (regime === 'base' || (regime === 'auto' && rec.time < EFFECTIVE_AT)) {
          const p = PRICING.base[mk]
          if (!p) return 0
          return (hit * p.cacheHit + miss * p.cacheMiss + out * p.output) / 1e6
        }
        const pv = peakValleyTableAt(rec.time)[mk]
        if (!pv) return 0
      ...
      async function routeApi(body) {
        ...
        switch (action) {
          case 'list': {
            ...
            return { ok: true, records: items, count: items.length, toolCalls: toolItems, dataPath, persistOk, persistError, pricing: PRICING, effectiveAt: EFFECTIVE_AT, effectiveV41At: V41_EFFECTIVE_AT, days: buildDays(), fx: FX, budget }
          }
